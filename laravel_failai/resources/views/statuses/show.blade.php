    ...
    <div class="col s12 m3">
        <div class="card">
            ...
            <div class="card-content">
                <div>{{__('names.id')}}: {{$status->id}}</div>
                <p>{{__('names.name')}}: {{ $status->name }}</p>
                <p>{{__('names.description')}}: {{$status->description}}</p>
            ...
            <div class="card-action">
                ...
                <form action="{{ route('statuses.destroy', $status->id) }}" method="POST">
                    ...
                    <a href="{{route('statuses.index')}}" class="btn btn-primary">{{__('action.home')}}</a>
                </form>
            </div>
        </div>
    </div>
